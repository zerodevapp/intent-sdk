...
export async function signSolanaInstructions(
  ...
  const feePayer = createNoopSigner(
    address("Data6X2sFNz6WFGJ5nXBCYMvyVvJUQh5oUJLkPd9pM58"),
  );
  ...
  const txWithSigner = addSignersToTransactionMessage(
    [solanaSigner],
    transactionMessage,
  );
  ...
  const signedTransaction =
    await partiallySignTransactionMessageWithSigners(txWithSigner);
